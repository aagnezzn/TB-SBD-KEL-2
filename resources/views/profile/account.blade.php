        ...
        <aside class="w-full md:w-1/4">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 mb-6">
                <div class="flex flex-col items-center">
                    ...
                    <p class="text-xs text-slate-500 mt-1 uppercase tracking-wider font-semibold">{{ __('account.personal_account') }}</p>
                </div>
            </div>
            ...
            <nav class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden p-2 space-y-1">
                <a href="{{ route('profile.public', auth()->id()) }}" class="group flex items-center px-4 py-3 rounded-xl transition-all duration-200 text-slate-600 hover:bg-slate-50 hover:text-slate-900">
                    <span class="text-sm font-semibold">{{ __('account.view_public_profile') }}</span>
                </a>
                ...
                <button @click="activeTab = 'profil'; window.history.replaceState(null, '', '?tab=profil')" type="button"
                    ...
                    <span class="text-sm">{{ __('account.profile') }}</span>
                </button>
                ...
                <button @click="activeTab = 'foto'; window.history.replaceState(null, '', '?tab=foto')" type="button"
                    ...
                    class="group flex items-center px-4 py-3 rounded-xl transition-all duration-200 text-left cursor-pointer">
                    <span class="text-sm">{{ __('account.photo') }}</span>
                </button>
                ...
                <button @click="activeTab = 'keamanan'; window.history.replaceState(null, '', '?tab=keamanan')" type="button"
                    ...
                    class="group flex items-center px-4 py-3 rounded-xl transition-all duration-200 text-left cursor-pointer">
                    <span class="text-sm">{{ __('account.account_security') }}</span>
                </button>
            </nav>
        </aside>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div x-show="activeTab === 'profil'" x-transition>
                    <div class="p-8 md:p-12 border-b border-slate-100 bg-white">
                        <h1 class="text-3xl font-black text-slate-900 tracking-tight">{{ __('account.public_profile') }}</h1>
                        <p class="text-slate-500 mt-2 text-base">{{ __('account.add_info_about_yourself') }}</p>
                    </div>
                    ...
                    <div class="p-8 md:p-12">
                        <form action="{{ route('account.profile.update') }}" method="POST" class="space-y-8">
                            ...
                            <div class="space-y-4">
                                <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest">{{ __('account.basics') }}</h3>
                                <input type="text" name="first_name" required value="{{ auth()->user()->profile->first_name ?? '' }}" placeholder="{{ __('account.first_name') }}" class="w-full bg-white border border-slate-200 p-4 rounded-xl focus:outline-none focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-semibold text-gray-700">
                                <input type="text" name="last_name" value="{{ auth()->user()->profile->last_name ?? '' }}" placeholder="{{ __('account.last_name') }}" class="w-full bg-white border border-slate-200 p-4 rounded-xl focus:outline-none focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-semibold text-gray-700">
                                
                                <div class="space-y-2">
                                    <div class="relative">
                                        <input type="text" name="headline" maxlength="60" value="{{ auth()->user()->profile->headline ?? '' }}" placeholder="{{ __('account.professional_headline') }}" class="w-full bg-white border border-slate-200 p-4 pr-12 rounded-xl focus:outline-none focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-semibold text-gray-700">
                                        <span class="absolute right-4 top-4 text-slate-400 text-xs font-bold">60</span>
                                    </div>
                                    <p class="text-xs text-slate-400 ml-1">{{ __('account.add_job_headline') }}</p>
                                </div>
                            </div>

                            <div class="space-y-3">
                                <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest">{{ __('account.short_bio') }}</h3>
                                <textarea name="bio" rows="4" placeholder="{{ __('account.tell_us_about_yourself') }}" class="w-full bg-white border border-slate-200 p-4 rounded-xl focus:outline-none focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-medium text-gray-700 leading-relaxed resize-none">{{ auth()->user()->profile->bio ?? '' }}</textarea>
                            </div>

                            <div class="space-y-6">
                                <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest">{{ __('account.social_links') }}</h3>
                                @php 
                                    $sosmeds = [
                                        'website'   => ['domain' => __('account.website_domain'), 'placeholder' => 'https://domain.com', 'hint' => __('account.website_hint')],
                                        'facebook'  => ['domain' => 'facebook.com/', 'placeholder' => 'username', 'hint' => __('account.facebook_hint')],
                                        'instagram' => ['domain' => 'instagram.com/', 'placeholder' => 'username', 'hint' => __('account.instagram_hint')],
                                        'twitter'   => ['domain' => 'x.com/', 'placeholder' => 'username', 'hint' => __('account.twitter_hint')]
                                    ]; 
                                @endphp
                ...
                <div x-show="activeTab === 'foto'" x-transition x-cloak>
                    <div class="p-8 md:p-12 border-b border-slate-100 bg-white">
                        <h1 class="text-3xl font-black text-slate-900 tracking-tight">{{ __('account.profile_photo') }}</h1>
                        <p class="text-slate-500 mt-2 text-base">{{ __('account.manage_profile_picture') }}</p>
                    </div>
                    ...
                    <div class="p-8 md:p-12">
                        ...
                        @csrf @method('PUT')
                        <div class="space-y-4">
                            <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest">{{ __('account.current_image_preview') }}</h3>
                        ...
                        <div class="space-y-4">
                            <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest">{{ __('account.add_change_new_image') }}</h3>
                            <div class="flex flex-col md:flex-row gap-3">
                                <input type="text" id="file-name-display" readonly placeholder="{{ __('account.no_file_selected') }}" class="flex-1 bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-semibold text-slate-500 focus:outline-none">
                        ...
                        @if(Auth::user()->profile && Auth::user()->profile->photo)
                            <form action="{{ route('account.avatar.delete') }}" method="POST" onsubmit="return confirm(`{{ __('account.confirm_delete_photo') }}`)" class="m-0 w-full md:w-auto">
                            </form>
                        @endif
                        </div>
                    </div>
                ...
                <div x-show="activeTab === 'keamanan'" x-transition x-cloak x-data="{ openEmailModal: false }">
                    <div class="p-8 md:p-12 border-b border-slate-100 bg-white">
                        <h1 class="text-3xl font-black text-slate-900 tracking-tight">{{ __('account.security_settings') }}</h1>
                        <p class="text-slate-500 mt-2 text-base">{{ __('account.manage_login_credentials') }}</p>
                    </div>

                    <div class="p-8 md:p-12 space-y-10">
                        <div class="space-y-4">
                            <label class="block text-xs font-black text-slate-400 uppercase tracking-widest">{{ __('account.email_account_address') }}</label>
                            <div class="flex flex-col sm:flex-row gap-3">
                                ...
                                <button @click="openEmailModal = true" type="button" class="bg-white border border-slate-200 text-slate-600 px-6 py-4 rounded-xl font-bold text-xs uppercase tracking-wider hover:bg-slate-50 cursor-pointer">{{ __('account.change') }}</button>
                            </div>
                        </div>
                        ...
                        <div x-show="openEmailModal" x-cloak x-transition class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-[1px]">
                            <div @click.away="openEmailModal = false" class="relative w-full bg-white rounded-2xl shadow-2xl mx-4 p-8 max-w-[620px]">
                                ...
                                <h2 class="text-xl font-black mb-2">{{ __('account.change_your_email') }}</h2>
                                <form action="{{ route('account.email.update') }}" method="POST">
                                    @csrf @method('PATCH')
                                    <input type="email" name="new_email" required placeholder="new-email@example.com" class="w-full border border-slate-200 p-4 rounded-xl mb-6 outline-none focus:border-purple-500 font-semibold text-sm">
                                    <div class="flex justify-end">
                                        <button type="submit" class="bg-purple-600 text-white px-6 py-3.5 rounded-xl font-bold text-xs uppercase tracking-widest cursor-pointer hover:bg-purple-700 shadow-md">{{ __('account.verify_new_email') }}</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                        ...
                        <form action="{{ route('account.password.update') }}" method="POST" class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-3">
                                    <label class="block text-xs font-black text-slate-400 uppercase tracking-widest">{{ __('account.new_password') }}</label>
                                ...
                                <div class="space-y-3">
                                    <label class="block text-xs font-black text-slate-400 uppercase tracking-widest">{{ __('account.confirm_new_password') }}</label>
                            ...
                            <button type="submit" class="bg-[#5624d0] text-white font-bold text-xs uppercase tracking-widest px-6 py-4 rounded-xl hover:bg-purple-900 shadow-md active:scale-95 cursor-pointer">{{ __('account.update_password') }}</button>
                        </form>
                    </div>
                </div>

                {{-- FOOTER TIMESTAMP DETAIL --}}
                <div class="p-6 text-center border-t border-slate-100 bg-slate-50/50">
                    <p class="text-slate-400 text-xs italic">
                        {{ __('account.last_updated') }} {{ auth()->user()->profile ? auth()->user()->profile->updated_at->diffForHumans() : __('account.just_now') }}
                    </p>
                </div>
