...
<div class="flex gap-6 border-b border-gray-100 pb-8 mb-6 last:mb-0">
    <a href="{{ route('course.show', $item->course->id) }}" class="shrink-0">
        <img src="https://loremflickr.com/150/100/tech?random={{ $item->course->id }}" class="w-32 h-20 object-cover rounded-md shadow-sm">
    </a>
...
<script>
    ...
    function slideLeft() {
        if (isMoving) return;
        const cardWidth = 276; // Lebar kartu + gap
        
        // Jika di paling awal, pindah instan ke arah akhir (loop)
        ...
        slider.scrollLeft -= cardWidth * 3; // Geser 3 kartu sekaligus
    }
    ...
    function slideRight() {
        ...
        // Jika mendekati ujung kanan, pindah instan ke tengah/awal
